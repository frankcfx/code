...
object GroupSortedTopN {
  def main(args: Array[String]): Unit = {
    ...
    /**
      * 思路：先按照key进行分组(groupByKey), 然后对分组之后的数据(key为单词，value为所有单词相同的数字组成的一个迭代器)进行处理，处理方式为：对迭代器中的数据进行排序操作；最后获取排序好的迭代器中的前K个值
      * NOTE: 一般情况下，要尽可能的减少RDD的调用链(如果调用链太多，就有可能导致task没法运行<task的序列化后的大小是有限制的，如果超过限制，那么driver没法将task发送给executor的>) ===> 这种问题的解决方案很简单：1. 合并API(合并map、flatMap、filter之类的API); 2. 在rdd中进行cache缓存或者checkpoint
      * 缺点1：groupByKey这个API在当前版本中，同组(相同)key对应的value数据会形成一个迭代器，并且这个迭代器在后续的处理中会全部的加载到内存中；所以如果一个key对应的value数据太多，就有可能出现数据倾斜或者OOM异常
      * 缺点2：在同组key数据进行聚合的业务场景中，groupByKey API的性能有点低，因为不会进行分区数据的局部聚合(不会降低shuffle的数据量)，而groupByKey API会将所有的数据都发送给下一个阶段(会进行shuffle操作)，在聚合的业务场景中，shuffle数据中其实有部分数据是无用的(不需要考虑) ==> 实际的聚合业务场景中，一般一个分区中只有部分数据需要考虑
      */
    /*mappedRDD
      .groupByKey()
      .map(t => {
    ...
    // driver中定义了一个random的变量，在executor中使用了该变量，所以要求random变量是可以进行序列化操作的(因为需要网络传输)； 但是这里的这个random变量不能序列化操作，所以不能传递，如果直接运行会报错: Exception in thread "main" org.apache.spark.SparkException: Task not serializable
    //    val random = Random
    ...
    /**
      ...
      * seqOp: 对每组key对应的value数据进行迭代处理，将value和之前的聚合值(U值)进行聚合操作，并最终返回一个新的聚合U值 ==> 在shuffle之前，对当前分区中对应key的value进行聚合操作的时候会被调用
      * combOp：对两个分区的聚合数据进行一个全局聚合操作，并返回一个新的聚合值 ==> 在shuffle发送数据之后，对多个分区的结果值进行聚合操作的时候会被调用；该API在数据量比较大，并且内存不够的情况下，有可能会在shuffle之前触发(首先将seqOp操作的结果输出磁盘，然后对磁盘的输出结果调用combOp进行合并操作 ---> 类似MapReduce中的combiner操作)
      */
