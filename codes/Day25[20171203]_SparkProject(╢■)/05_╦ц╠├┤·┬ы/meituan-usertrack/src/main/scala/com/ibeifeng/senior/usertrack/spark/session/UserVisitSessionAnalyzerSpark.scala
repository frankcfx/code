...
object UserVisitSessionAnalyzerSpark {
  def main(args: Array[String]): Unit = {
    ...
    val sqlContext = SQLContextUtil.getInstance(
      sc,
      integratedHive = !isLocal, //本地运行不需要集成，集群运行需要集成
    ...
    // 四、数据按照session聚合
    // 按照session id聚合，将一个session的所有数据放到一起；也就是一条RDD的数据记录一个会话的所有数据
    // TODO: 这里使用groupByKey API进行聚合，原因：-1. 业务需要，必须使用；-2. 一个会话的数据不会太多，不可能出现数据倾斜或者OOM的异常 ==> 所以这里使用groupByKey API问题不大
    ...
    val ratio = {
      // 获取参数
      val param = ParamUtils.getParam(taskParam, Constants.PARAM_SESSION_RATIO)
    ...
    val top10CategoryIDAndCountRDD: RDD[(String, (Int, Int, Int))] = sessionID2RecordsRDD
      .flatMap {
        case (sessionID, records) => {
          // 从records迭代器中获取各个操作触发的品类id， 类型: Iterable[(品类id，操作类型)] =>
          // 0表示点击，1表示下单，2表示支付
        ...
        case (flag, iter) => {
          ...
          top10Category.map {
            case (categoryID, count) => {
              // 有某个品类(categoryID)，进行了某个操作(flag)，操作了多少次(count)
              (categoryID, (flag, count))
            }
          }
        }
      }
      //      .repartition(1) // 因为这里实际上不需要shuffle操作的，所以不要使用repartition这个API, 而是使用coalesce API
      .coalesce(1)
      .mapPartitions(iter => {
        // 因为在mapPartitions API之前做了repartition操作，并且repartition的分区数量为1
        // 所以这里的iter表示所有的数据 ==> 最多30条
        iter
          .toList
          .groupBy(_._1)
          .map {
            case (categoryID, list) => {
              ...
              // 类型: (点击的数量，下单的数据，支付的数量)
              val categoryCount: (Int, Int, Int) = list.foldLeft((0, 0, 0))((a, b) => {
                b._2._1 match {
  ...
  /**
    * 根据过滤参数获取对应的数据，并将数据封装成为RDD
    * 应该支持：
    * 时间、年龄、性别、地域、关键词的过滤、职业、最近消费额、最近订单数....
    *
    * @param sqlContext 上下午
    * @param taskParam  过滤参数对象
    * @return 返回一个RDD
    */
