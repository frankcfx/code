...
object MyKafkaUtils {
  ...
  val zkQuorum = "zookeeper1:2181,zookeeper2:2181,zookeeper3:2181/kafka07"

  // 初始化Zookeeper客户端
  val zkClient = {
    val client = CuratorFrameworkFactory.builder.connectString(zkQuorum).
  ...
  def getZKOffsets(topicSet:Set[String], groupName:String, kafkaParam: Map[String, String]) : Map[TopicAndPartition, Long] = {
    ...
    for{
      ...
      // if (zkClient.checkExists().forPath(partionPath) != null) zkClient.getData.forPath(partionPath) else "-1"
      val offset =  zkClient.getData.forPath(partionPath)
  ...
  def getConSumerOffsets(kafkaParam: Map[String, String], topicSet:Set[String], groupName:String) : Map[TopicAndPartition, Long] = {
    ...
    val smallOffsets: Map[TopicAndPartition, Long] = getResetOffsets(kafkaSmallestParams, topicSet)
  ...
  def main(args: Array[String]): Unit = {
    ...
    val largestOffsets: Map[TopicAndPartition, Long] = getResetOffsets(kafkaParamsLargest, topicsSet)
