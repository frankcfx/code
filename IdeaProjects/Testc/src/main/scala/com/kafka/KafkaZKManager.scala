...
object KafkaZKManager extends Serializable {
  ...
  val client = {
    val client = CuratorFrameworkFactory
      .builder
      .connectString("zookeeper1:2181,zookeeper2:2181,zookeeper3:2181/kafka07")
...
//    val t: util.List[String] = client.getChildren.forPath(zkTopicPath)
    val offsets = for {
  ...
  def createMyDirectKafkaStream(ssc: StreamingContext, kafkaParams: Map[String, String], topic: String, groupName: String
    ...
    if (flag == 1) {
      ...
      // 如果未保存,根据kafkaParam的配置使用最新(greatest)或者最旧(smallest)的offset
